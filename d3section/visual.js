...
d3.csv("100_directors.csv").then((data) => {
    // var r = new FileReader();
    // r.readAsText(this.files[0], config.encoding);
    // r.onload = function () {
        // var data = d3.csvParse(this.result);

        var allGroup = d3.map(data, function (d) { return (d.type) }).keys().sort()
        ...
        //var allGroup1 = d3.map(data, function (d) { return (d.date) }).keys()
        ...
        function update(selectedGroup) {
            var filterdata = data.filter(function (d) { return d.type == selectedGroup })
            draw(filterdata)
        }
        ...
        var allGroup1 = d3.map(data, function (d) { return (d.date) }).keys().sort()
        d3.select("#selectButton1")
            .selectAll('myOptions')
            .data(allGroup1)
            .enter()
        ...
        function update1(selectedGroup) {
            var filterdata = data.filter(function (d) { return d.date == selectedGroup })
            draw(filterdata)
        }
...
function draw(data) {
    ...
    var date = [];
    //var df = data.filter(function (d) { return d.date == allGroup[0] })
    ...
    if (dateLabel_x == null || dateLabel_y == null) {
        dateLabel_x = innerWidth; //default
        dateLabel_y = innerHeight; //default
    } //hide?
    ...
    if (showMessage) {
        ...
        // count the existing time for the top one 
        if (use_counter == true) {
            var days = g
                .insert("text")
                .attr("class", "days")
    ...
    function redraw() {
        ...
        //     .range([innerHeight, 0]);
        // range of x axis 
        // if all the number is too large, we won't see the differences 
        ...
        if (showMessage) {
            ...
            if (use_counter == true) {
                // update the time of the top one 
                days
                    .data(currentData)
                    .transition()
                    .duration(baseTime * interval_time)
...
function newdraw(data) {
    ...
    var date = [];
    var df = data.filter(function (d) { return d.type == allGroup[0] })
    df.forEach(element => {
        if (date.indexOf(element["date"]) == -1) {
            date.push(element["date"]);
        }
